...
var file = require('./file_modif');
var file = require('./file');
var params = require('./params');
var helper = require('./helper');
var format = require('./format');
var builder = require('./builder_modif');
...
function walkFiles (template, data, options, currentIndex, callback) {
  ...
  else if (_file.isMedia===true){
    builder.buildMedia(_file, data, options, function (err, replacedBuffer) {
      if (err) {
        return callback(err, template);
      }
      _file.data = replacedBuffer;
      process.nextTick(function () {
        walkFiles(template, data, options, ++currentIndex, callback);
      });
    });
