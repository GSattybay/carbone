...
var builder = {
  ...
  /**
   * Build the word/media/.. according to the content of data images
   * @param {Buffer } file   content of the embedded file in word/media
   * @param {Object|Array} data {images:[{"fileName":"12124.png", "base64":"sdfsdsdgs"},{"fileName":"1345345.png", "base64":"sdfsdsdgs"}]}
   * @param {Object} options {
   *                                  'complement'        : data which accessible with the market {c.}
   *                                  'formatters'        : formatters
   *                                  'lang'              : selected lang, ex. "fr"
   ...
   * @param {Function} callback
   */
  buildMedia: function(file, data, options, callback){
    if(typeof(options)==='function'){
      callback = options;
      options = {};
    }
    ...
    try {
      ...
      else { // compare data.images[i].name with file.name if equal replace file.data with data.images[i].content
        for (var i = 0; i < data.images.length; i++){
          if (file.name.toLowerCase()===imageFileTypes+data.images[i].fileName.toLowerCase() && data.images[i].base64){
            var buff = new Buffer(data.images[i].base64,'base64');
